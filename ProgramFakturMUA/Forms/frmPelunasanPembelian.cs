...
{
    public partial class frmPelunasanPembelian : Form
    {

        frmLaporanDaftarUtang frmLaporanDaftarUtang;
        string id_beli;
        ...
        public frmPelunasanPembelian(frmLaporanDaftarUtang frm, string id)
        {
            InitializeComponent();
            id_beli = id;
            frmLaporanDaftarUtang = frm;
        }

        private void frmPelunasanPembelian_Load(object sender, EventArgs e)
        {
            this.Location = new Point(0, 0);

            string lunas = pembelian.getPembelian(id_beli, "lunas");
            string tanggal_lunas = pembelian.getPembelian(id_beli, "tanggal_lunas");
            string no_faktur = pembelian.getPembelian(id_beli, "no_faktur");
            string supplier_id = pembelian.getPembelian(id_beli, "supplier_id");
            string nama_supplier = mitrabisnis.getMitraBisnis(supplier_id, "nama_supplier");
            string grandtotal = pembelian.getPembelian(id_beli, "tagihan");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            pembelian.updatePembelian(id_beli, "lunas", lunas.ToString());
            pembelian.updatePembelian(id_beli, "tanggal_lunas", dateTimePicker1.Value.ToString("yyyy/MM/dd"));
            fungsi.showSuccess("Status pelunasan berhasil disimpan");
            frmLaporanDaftarUtang.refreshData();
        }
        ...
        private void btnHapus_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                {
                    pembelian.hapusPembayaran(dataGridView1.SelectedRows[0].Cells[0].Value.ToString());
                    showData();
                }
            }
        }
        ...
        private void btnSimpan_Click(object sender, EventArgs e)
        {
            ...
            {
                pembelian.insertPembayaran(id_beli, dateTimePicker1.Value.ToString("yyyy/MM/dd"), txtJumlahBayar.Text);
                showData();
                txtJumlahBayar.Text = "";
            }
        }
